...
def MakeData(number_of_students, actref_dist,senint_dist,visver_dist,seqglo_dist):
	...
	for i in range(number_of_students):
		...
		act_ref = int(np.random.normal(actref_dist[0],actref_dist[1]))
		if act_ref > 11:
			act_ref = 11
		elif act_ref < -11:
			act_ref = -11

		sen_int = int(np.random.normal(senint_dist[0], senint_dist[1]))
		if sen_int > 11:
			sen_int = 11
		elif sen_int < -11:
			sen_int = -11

		vis_ver = int(np.random.normal(visver_dist[0], visver_dist[1]))
		if vis_ver > 11:
			vis_ver = 11
		elif vis_ver < -11:
			vis_ver = -11
		print int(np.random.normal(seqglo_dist[0], seqglo_dist[1]))	

		seq_glo = int(np.random.normal(seqglo_dist[0], seqglo_dist[1]))
		if seq_glo > 11:
			seq_glo = 11
		elif seq_glo < -11:
			seq_glo = -11
